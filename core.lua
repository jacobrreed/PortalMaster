...
-- Helper function for getting spell info
...
function Addon:OpenFrame(prefill)
	...
	if PortalMasterFrame:IsShown() then
		return
	end
...
function Addon:CreateTemporaryBinding()
	if Addon.TemporaryBindExists then
		return
	end
...
function Addon:RemoveTemporaryBinding()
	if not Addon.TemporaryBindExists then
		return
	end
